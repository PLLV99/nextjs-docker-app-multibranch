...
pipeline {
    ...
    environment {
        ...
        DOCKER_REPO               = 'sakamotolv99/nextjs-docker-app'
    ...
    parameters {
        ...
        string(
            name: 'ROLLBACK_TAG',
            defaultValue: '',
            description: 'For Rollback: enter the image tag to use (for example, Git hash or dev-123)'
        )
        choice(
            name: 'ROLLBACK_TARGET',
            choices: ['dev', 'prod'],
            description: 'For Rollback: choose which environment to rollback'
        )
    }
        ...
        stage('Execute Rollback') {
            ...
            post {
                success {
                    sendNotificationToN8n(
                        'success',
                        "Rollback ${params.ROLLBACK_TARGET.toUpperCase()}",
                        params.ROLLBACK_TAG,
                        env.ROLLBACK_APP_NAME ?: 'N/A',
                        env.ROLLBACK_HOST_PORT ?: 'N/A'
                    )
                }
            }
        }
